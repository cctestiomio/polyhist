...
async function fetchAllCashBalances(address) {
  ...
  balances.totalCash = (balances['USDC.e'] || 0) + (balances['USDC'] || 0) + (balances['pUSD'] || 0);
  return balances;
}
...
async function fetchClosedPositions(address) {
  ...
  while (true) {
    const batch = await fetchJSON(
      `${DATA_API}/closed-positions?user=${address}&limit=${limit}&offset=${offset}&sortBy=TIMESTAMP&sortDirection=DESC`
    );
...
function calculatePnl(openPositions, closedPositions = []) {
  const open = Array.isArray(openPositions) ? openPositions : [];
  const closed = Array.isArray(closedPositions) ? closedPositions : [];
  const openKeys = new Set(open.map(positionKey));
  const closedOnly = closed.filter((position) => !openKeys.has(positionKey(position)));

  const unrealized = open.reduce((sum, p) => sum + Number(p.cashPnl || 0), 0);
  const realizedOpen = open.reduce((sum, p) => sum + Number(p.realizedPnl || 0), 0);
  const realizedClosed = closedOnly.reduce((sum, p) => sum + Number(p.realizedPnl || 0), 0);
  const realized = realizedOpen + realizedClosed;
...
async function buildCurrentMetrics() {
  const results = await Promise.all(
    WALLETS.map(async (address) => {
      const [positions, closedPositions, cashBreakdown, positionValue] = await Promise.all([
        fetchPositions(address),
        fetchClosedPositions(address).catch((error) => {
          console.warn(`Closed positions fetch failed for ${address}:`, error?.message || error);
          return [];
        }),
      ...
      return {
        ...
        closedPositions: Array.isArray(closedPositions) ? closedPositions : [],
  ...
  const allClosedPositions = results.flatMap((item) => item.closedPositions);
  ...
  const pnl = results.reduce((totals, item) => {
    const walletPnl = calculatePnl(item.positions, item.closedPositions);
    totals.unrealized += walletPnl.unrealized;
    totals.realized += walletPnl.realized;
    totals.closedCount += walletPnl.closedCount;
    return totals;
  }, { unrealized: 0, realized: 0, closedCount: 0 });
  const totalUnrealized = pnl.unrealized;
  const totalRealized = pnl.realized;
  ...
  return {
    ...
    closedPositionCount: allClosedPositions.length,
    wallets: results.map((item) => ({
      address: item.address,
      cash: Number(item.cash || 0),
      positions: Number(item.positionValue || 0),
      closedPositions: Array.isArray(item.closedPositions) ? item.closedPositions.length : 0,
      usdc_e: Number(item.cashBreakdown?.['USDC.e'] || 0),
      usdc: Number(item.cashBreakdown?.USDC || 0),
      pusd: Number(item.cashBreakdown?.pUSD || 0)
    }))
  };
}
